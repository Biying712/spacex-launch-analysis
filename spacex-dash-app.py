...
def get_scatter_chart(entered_site,payload_range):
    minpay,maxpay=payload_range
    filtered_df = spacex_df[(spacex_df['Payload Mass (kg)'] >= minpay) &
                            (spacex_df['Payload Mass (kg)'] <= maxpay)]
    if entered_site == 'ALL':
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', 
        y='class', 
        color="Booster Version Category",
        title='Correlation between payload mass and success')
        return fig
    else:
        # return the outcomes piechart for a selected site
